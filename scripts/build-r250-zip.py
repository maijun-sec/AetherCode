"""R250 release: build aethercode-0.2.63.zip."""
import sys
import zipfile
from pathlib import Path

RELEASE_ROOT = Path(r"D:\work\workspace\idea\engine\AetherCode\release")
SRC_DIR = RELEASE_ROOT / "aethercode-0.2.63"
OUT_ZIP = RELEASE_ROOT / "aethercode-0.2.63.zip"
EXCLUDE_NAMES = {"_trash_r250", ".DS_Store"}
...
def main() -> int:
    ...
    with zipfile.ZipFile(OUT_ZIP, "w", zipfile.ZIP_DEFLATED, compresslevel=6) as zf:
        for src, rel in files:
            arc = "aethercode-0.2.63/" + rel.as_posix()
            zf.write(src, arc)
