...
/** Snapshot of a backstop attempt — used by the TUI and tests. */
export type BackstopAttempt = {
  /** Which attempt this was: 0 = original, 1 = retry. */
  attempt: number;
  /** What triggered the retry. */
  trigger: "prompt_too_long";
  /** Whether the attempt succeeded. */
  succeeded: boolean;
  /** Elapsed ms of the attempt (original or retry). */
  elapsedMs: number;
  /** The Layer 1 result that produced the shrunken input. */
  compactResult: CompactResult | null;
  /** Error message when the attempt failed. */
  error?: string;
};

/** Outcome of `llmCallWithBackstop` for a single call. */
export type BackstopOutcome = {
  /** The LLM response (original or the retry's response). */
  text: string;
  /** The model name the LLM was called with (passthrough). */
  model: string;
  /**
   * Backstop attempts, in order. Empty when no `prompt_too_long`
   * occurred. The last entry is the successful retry; earlier
   * entries describe the original failure.
   */
  attempts: BackstopAttempt[];
};

/** Match patterns for the `prompt_too_long` API error. */
const PROMPT_TOO_LONG_PATTERNS: ReadonlyArray<RegExp> = [
...
/** Per-call options for `llmCallWithBackstop`. */
export type ReactiveBackstopOptions = {
  /** Pipeline options — used to derive Layer 1 thresholds. */
  pipelineOptions: CompactPipelineOptions;
  /**
   * Optional circuit breaker. When provided, a failed retry calls
   * `recordFailure` on the breaker so the TUI can surface the
   * persistent warning and stop further auto-compaction.
   */
  circuitBreaker?: CircuitBreaker;
  /** Optional session id used by the circuit breaker. */
  sessionId?: string;
 ...
 */
export async function llmCallWithBackstop(
  args: {
    input: CompactInput;
    callOptions: { maxTokens?: number; model?: string };
    llm: LlmClient;
  },
  options: ReactiveBackstopOptions,
): Promise<BackstopOutcome> {
  const now = options.now ?? Date.now;
  const attempts: BackstopAttempt[] = [];
  ...
  const shrunkCompactResult: CompactResult = {
    layer: 1,
    beforeTokens: 0,
...
export class ReactiveBackstopExhaustedError extends Error {
  readonly attempts: ReadonlyArray<BackstopAttempt>;
  constructor(attempts: ReadonlyArray<BackstopAttempt>) {
 ...
 */
export function buildShrunkCompactInput(
  input: CompactInput,
  pipelineOptions: CompactPipelineOptions,
  options: { now?: () => number } = {},
): CompactInput {
 ...
 */
export async function reactiveBackstopPass(
  input: CompactInput,
  pipelineOptions: CompactPipelineOptions,
  options: {
    llm?: LlmClient;
  ...
  try {
    ...
    if (options.circuitBreaker && options.sessionId) {
      options.circuitBreaker.recordFailure(
        options.sessionId,
        isPromptTooLong(err) ? "backstop_retry_exhausted" : "llm_error",
      );
    }
    throw err;
  }
