...
// R312: ssdDriverRef removed — Spec-Driven Development is now
// driven directly by the Mavis agent in chat (no separate
// driver / chip strip / phase state machine). Users ask the
// agent to drive SDD in chat; the agent reads the spec-kit
// templates, calls the chat LLM, and writes the artefacts to
// `<cwd>/.aethercode/sdd/<slug>/` directly.
...
/**
 * R312: friendlySsdSpawnError removed — Spec-Driven
 * Development no longer spawns a jar subprocess. Users
 * drive SDD directly through the Mavis agent in chat
 * (see doc/user-guide/SDD.md). The daemon is still around
 * for the regular chat / a2a / workflow flows but SDD
 * has been pulled out of the desktop binary entirely.
 */
...
interface AppState {
  ...
  // R312: sddEnabled removed (Spec-Driven Development is now
  // driven by the Mavis agent in chat; see
  // doc/user-guide/SDD.md).
  // R312: sddEnabled / setSddEnabled / ssdPhases / ssdActive /
