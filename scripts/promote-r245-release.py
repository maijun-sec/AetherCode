"""对应历史 round.3 engineering milestone: assemble release/aethercode-0.2.61.

Contents (mirroring rmem6 release):
  - desktop/aethercode-desktop.exe (latest src-tauri build)
  - desktop/resources/ (jar + icon)
  - aethercode-0.2.61.jar (canonical CLI shaded)
  - ac-tui, ac-tui-standalone.exe, README.md, run-tui.bat, run-tui.sh
    (copied from previous release; TUI bundle is the same; we only
     ship the engine + desktop)
  - RELEASE-NOTES-r245.md
"""
import os
import shutil
import sys
...
PREV_RELEASE = ROOT / 'release' / 'aethercode-0.2.60'
DST_DIR = ROOT / 'release' / 'aethercode-0.2.61'
SRC_JAR = ROOT / 'aethercode' / 'dist' / 'aethercode-0.2.61.jar'
...
# 3) Canonical jar
dst_jar = DST_DIR / 'aethercode-0.2.61.jar'
