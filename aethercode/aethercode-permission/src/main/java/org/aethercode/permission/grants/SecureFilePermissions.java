 ...
 * no-op there and a separate ACL is recommended at
 * install time. The 0600-permission call is therefore
 * best-effort and never throws — a failure to set
 * POSIX bits is logged at warn but does not break the
 * write (the data is still on disk, just slightly less
 * locked down than the canonical config). Users who
 * want hard enforcement can wrap this helper with a
...
public final class SecureFilePermissions {
    ...
    static {
        boolean supported;
        try {
            // Touch a known POSIX-only API; on Windows this
            // throws UnsupportedOperationException. We
            // cache the result so the per-write branch is
            // a single volatile read.
