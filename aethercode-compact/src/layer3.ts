...
import { compactSummarySchema, MAX_SEGMENT_CHARS, SEGMENT_FIELD_NAMES, type CompactSummarySchema } from "./schema.js";
...
/** Per-call options for Layer 3. */
export type Layer3Options = {
  /** Max output tokens the model can emit. */
  maxOutputTokens: number;
  /** Optional override of the 4-shot prompt template. */
  promptTemplate?: string;
  /**
   * Number of recent user messages to keep verbatim after Layer 3
   * replaces the rest of the history. Default: 5.
   */
  recentKeep: number;
  /**
   * Disable history replacement for this call. When true the returned
   * `history` is the input history, identical to the Round-1 behavior.
   * Used by tests that focus on the summary shape. Default: false.
   */
...
export function buildSchemaReminder(): string {
  const lines = [
    "{",
...
export function renderHistoryForPrompt(history: ReadonlyArray<Message>): string {
  const out: string[] = [];
...
export function parseAndValidate(raw: string): CompactSummarySchema {
...
export function describeSchema(): { name: string; fields: string[] } {
  return {
    name: "CompactSummary",
 ...
 */
export function enforceSegmentCap(summary: CompactSummary): CompactSummary {
 ...
 */
function buildSummaryMessage(summary: CompactSummary, ts: number): Message {
 ...
 * returned.
 */
export function replaceHistoryKeepingRecent(
  history: ReadonlyArray<Message>,
 ...
 */
export function assertSchemaShape(): void {
  const sample: CompactSummarySchema = {
    userTaskIntent: "x",
    projectContext: "x",
    approachTaken: "x",
    bugsAndFailures: "x",
