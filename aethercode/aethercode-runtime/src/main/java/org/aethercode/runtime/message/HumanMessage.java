 ...
 *
 * <p>Mirror of langchain's <code>HumanMessage</code>. The content can be
 * either a plain string (most common) or a list of {@link ContentBlock}
 * for multimodal input.</p>
 */
public record HumanMessage(
    ...
    public HumanMessage {
        if (content == null) {
            throw new IllegalArgumentException("HumanMessage.content is required");
        }
    ...
    public HumanMessage(List<ContentBlock> blocks) {
        this(java.util.UUID.randomUUID().toString(),
             Optional.empty(),
             blocks,
             blocks);
