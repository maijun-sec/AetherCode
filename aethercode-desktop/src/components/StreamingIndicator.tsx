 ...
 * matching the existing top-of-page ActivityIndicator palette
 * so the user's eye learns the language once.
 */
...
export function StreamingIndicator({
  ...
  //   2) isStreaming is true and the caller wants the placeholder
  //      (e.g. just-submitted-but-no-events-yet)
  const hasActivity = !!activity;
  if (!hasActivity && !(isStreaming && forceWhenEmpty)) return null;
  ...
  if (variant === 'empty' && activity && (activity.kind === 'done' || activity.kind === 'error')) {
    return null;
  }
