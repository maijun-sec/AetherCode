...
/** Per-call options for Layer 1. */
export type Layer1Options = {
  /** Byte threshold for clearing a tool result body. */
  clearThresholdBytes: number;
  /** Warm window in ms. */
  cacheWarmWindowMs: number;
  /**
   * Override the system clock; useful for tests. Defaults to `Date.now`.
   */
  now?: () => number;
};

/** Decide whether the local cache is "warm" given the input. */
export function isCacheWarm(input: CompactInput, cacheWarmWindowMs: number, now: () => number = Date.now): boolean {
...
export function isReadClassTool(toolName: string | undefined): boolean {
  if (!toolName) {
    return false;
  }
...
export function shouldClearBody(body: string, thresholdBytes: number): boolean {
 ...
 */
function clearMessageBody(message: Message): Message {
...
function clearToolResultBody(result: ToolResult): ToolResult {
 ...
 * directive.
 */
function collectClearedIds(messages: ReadonlyArray<Message>): string[] {
  const out: string[] = [];
 ...
 */
export function buildCacheReference(messages: ReadonlyArray<Message>): string {
...
export function microCompact(
  ...
  if (warm) {
    ...
    const cacheReference =
      warmIds.length > 0 ? `cache_reference:drop=${warmIds.join(",")}` : null;
    return {
      layer: 1,
      beforeTokens,
 ...
 */
function collectWarmReadClassIds(
